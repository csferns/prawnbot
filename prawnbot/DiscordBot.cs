...
namespace prawnbot
{
    public partial class DiscordBot : Form
    {
        ...
        private async Task HandleCommandAsync(SocketMessage arg)
        {
            ...
            if (message.HasStringPrefix("p!", ref argPos) || message.HasMentionPrefix(Client.CurrentUser, ref argPos) || context.IsPrivate)
            {
                if (context.Guild != null)
                {
                    ...
                    if (result.ErrorReason.Contains("Unknown")) await context.Channel.SendMessageAsync(result.ErrorReason);
                }
                else
                {
        ...
        private async void ConnectBot()
        {
            ...
            try
            {
                Assembly commandsAssembly = typeof(Commands).Assembly;
                await Commands.AddModulesAsync(Assembly.Load(commandsAssembly.FullName), Services);
